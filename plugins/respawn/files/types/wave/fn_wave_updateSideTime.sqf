/* ----------------------------------------------------------------------------
Function: respawn_fnc_wave_updateSideTime
...
Description:
	...
	"respawn_waveDelay" - sets the time for the respawn window
	"respawn_waveCount" - from paraameters used for the max waves allowed
Parameters:
0:	_side	- side to update
Returns:
	nothing
Examples:
	west call respawn_fnc_wave_updateSideTime;
Author:
	nigel
---------------------------------------------------------------------------- */
...
if (_requested) then {
	...
	private _defaultTime = 60 * (missionNamespace getVariable ["respawn_waveTime",RESPAWN_PARAM_WAVETIME]);
	...
	private _delay = missionNamespace getVariable ["respawn_waveDelay",RESPAWN_PARAM_WAVEDELAY];
